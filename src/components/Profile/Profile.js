...
const Profile = ({ }) => {
    ...
    function getPayment() {
        axios.get(`http://14.225.205.66/api/cart/read_payment.php?getIDUser=${getIDUser}`).then(function (response) {
            setPayment(response.data.data);
        });
    }
    useEffect(() => {
        axios.get(`http://14.225.205.66/api/users/read_user.php?getIDUser=${getIDUser}`)
