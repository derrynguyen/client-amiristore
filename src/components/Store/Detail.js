...
const Detail = ({ props }) => {
    ...
    useEffect(() => {
        axios.get(`https://14.225.205.66/Server/api/products/detail.php?id=${id}`)
    ...
    const addCart = async (name_brand, name, img, price, color, session) => {
        ...
        try {
            const response = await axios.post('https://14.225.205.66/Server/api/cart/addcart.php', formData);
